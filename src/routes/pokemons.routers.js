...
pokemonsRouter.post("/", async (req, res) => {
  ...
  const pokemon = await pokemonController.postPokemon(
    name,
    hp,
    attack,
    defense,
  ...
  return res
    .status(201)
    .json({ message: "pokemon cadastrado", status: 201, id: pokemon });
});
...
pokemonsRouter.put("/:id", async (req, res) => {
  ...
  const pokemon = await pokemonController.putPokemon(
    pokemon_id,
    name,
    hp,
    attack,
